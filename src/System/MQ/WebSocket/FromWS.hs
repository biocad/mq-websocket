...
import           System.Log.Logger              (errorM, logM)
...
import           System.MQ.WebSocket.Connection (ClientId, packConnection, WSConnection (..),
                                                 removeConnection,
...
acceptConnection pending clientId = do
    ...
    -- logM websocketName "New connection established"
    wsConnection <- packConnection clientId connection

    finally (dispatchClientMessage wsConnection) (removeConnection wsConnection)
...
dispatchClientMessage wsConnection@(WSConnection _ _ connection) = runMQMonad $ do
    ...
    foreverSafe websocketName $ do
        ...
        let m = unpack packedMessage :: Maybe WSData
        liftIO $ print m
